    ...
    <>
      ...
      <motion.header
        ...
        className={`sticky top-0 z-50 backdrop-blur-xl border-b-2 transition-all duration-300 ${
          isScrolled ? 'bg-black/95 border-white/20 shadow-lg shadow-white/5' : 'bg-black border-white/10'
        }`}
      >
        ...
        <div className="container mx-auto px-4 md:px-6 py-3">
          <div className="flex items-center justify-between lg:justify-center gap-4 md:gap-6 max-w-7xl mx-auto">
            ...
            <button
              ...
              className="lg:hidden p-2 text-white hover:text-white/80 transition-colors duration-200 order-1"
            ...
            <Link href="/" className="flex items-center group flex-shrink-0 order-2 lg:order-1">
              <div className="relative w-36 h-11 md:w-40 md:h-12 transition-all duration-300 group-hover:scale-105">
                <OptimizedImage
                  ...
                  priority
                />
                <div className="absolute inset-0 bg-white/0 group-hover:bg-white/5 transition-all duration-300 blur-xl" />
              </div>
            </Link>
            ...
            <div className="hidden lg:flex flex-1 max-w-xl order-3 lg:order-2" ref={searchRef}>
              <div className="relative w-full">
                <input
                  ...
                  className="w-full px-4 py-3 pr-12 bg-white/10 border-2 border-white/20 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-white/30 transition-colors duration-200"
                />
                <button
                  onClick={handleSearch}
                  className="absolute right-0 top-0 h-full px-4 bg-white text-black rounded-r-lg hover:bg-gray-light transition-colors duration-200"
                  aria-label="Buscar"
                >
                  <Search size={20} />
                </button>
                ...
                <AnimatePresence>
                  ...
                  {isSearching && searchQuery.trim().length > 1 && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -10 }}
                      className="absolute top-full left-0 right-0 mt-2 bg-gray-dark border-2 border-white/20 rounded-lg shadow-xl z-50"
                    >
                  ...
                  {showSearchResults && !isSearching && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -10 }}
                      className="absolute top-full left-0 right-0 mt-2 bg-gray-dark border-2 border-white/20 rounded-lg shadow-xl overflow-hidden z-50"
                    >
                      {searchResults.length > 0 ? (
                        <>
                          {searchResults.map((product) => (
                            ...
                            >
                              <div className="relative w-12 h-12 bg-white/10 rounded flex-shrink-0">
                                <OptimizedImage
                          ...
                          <button
                            onClick={handleSearch}
                            className="w-full px-4 py-3 bg-white/5 hover:bg-white/10 text-white font-semibold text-sm transition-colors"
                          >
            ...
            <div className="flex items-center gap-2 md:gap-3 flex-shrink-0 order-3 lg:order-3">
              ...
              <Link
                href="/favoritos"
                className="relative p-2 text-white hover:text-white/80 transition-colors duration-200 group"
                aria-label="Favoritos"
              >
                <Heart size={24} className={wishlistItemCount > 0 ? 'fill-white text-white' : ''} />
                {wishlistItemCount > 0 && (
              ...
              >
                ...
                {cartItemCount > 0 && (
                  <span className="absolute -top-1 -right-1 bg-white text-black text-[11px] font-bold w-6 h-6 flex items-center justify-center rounded-full shadow-lg shadow-white/20">
                    {cartItemCount}
                  </span>
                )}
              </Link>
            ...
            {menuItems.map((item) => (
              ...
              >
                {item.hasSubmenu ? (
                  <>
                    <button
                      className="flex items-center gap-1.5 px-3 py-2 text-white font-black text-[13px] tracking-wider uppercase hover:text-white/80 transition-colors duration-200"
                    >
                      {item.name}
                    ...
                    <AnimatePresence>
                      {activeSubmenu === item.name && (
                        <motion.div
                          ...
                          className="absolute top-full left-0 mt-2 w-56 bg-black/95 backdrop-blur-md border-2 border-white/20 rounded-lg shadow-xl py-2 z-50"
                        >
                          {item.submenu.map((subItem) => (
                            <Link
                              key={subItem.name}
                              href={subItem.href}
                              className="block px-4 py-2.5 text-white text-sm font-semibold hover:bg-white/10 hover:text-white transition-colors duration-200"
                            >
                              {subItem.name}
                            </Link>
                          ))}
        ...
        {mobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 left-0 bottom-0 w-full max-w-sm border-r-2 z-[70] lg:hidden overflow-y-auto bg-black border-white/20"
            >
              <div className="flex flex-col h-full">
                ...
                <div className="flex items-center justify-between p-4 pb-3">
                  ...
                  <button
                    onClick={() => setMobileMenuOpen(false)}
                    className="p-2 text-white hover:text-white/80 transition-colors -ml-2"
                  ...
                  <div className="flex items-center gap-2">
                    <Link
                      ...
                      aria-label="Favoritos"
                    >
                      <Heart size={20} className={wishlistItemCount > 0 ? 'fill-white text-white' : ''} />
                      {wishlistItemCount > 0 && (
                        <span className="absolute -top-1 -right-1 bg-white text-black text-[10px] font-black w-4 h-4 flex items-center justify-center rounded-full">
                          {wishlistItemCount}
                        </span>
                      )}
                    </Link>
                      ...
                      {item.hasSubmenu ? (
                        <div className="w-full">
                          <button
                            ...
                            className="flex items-center justify-between w-full px-4 py-3 text-white hover:text-white/80 text-base font-bold uppercase tracking-wide transition-colors duration-200 hover:bg-white/5 rounded-lg"
                          >
                            {item.name}
                          ...
                          <AnimatePresence>
                            {activeSubmenu === item.name && (
                              <motion.div
                                ...
                                transition={{ duration: 0.2 }}
                                className="overflow-hidden ml-4 mt-2 space-y-1 bg-black/40 backdrop-blur-sm rounded-lg p-2"
                              >
                                {item.submenu.map((subItem) => (
                                  <Link
                                    key={subItem.name}
                                    href={subItem.href}
                                    onClick={() => setMobileMenuOpen(false)}
                                    className="block px-4 py-2 text-white/80 hover:text-white text-sm font-medium transition-colors duration-200 hover:bg-white/10 rounded-lg"
                                  >
                                    {subItem.name}
                                  </Link>
                                ))}
                        ...
                        <Link
                          href={item.href}
                          onClick={() => setMobileMenuOpen(false)}
                          className="flex items-center px-4 py-3 text-white hover:text-white/80 text-base font-bold uppercase tracking-wide transition-colors duration-200 hover:bg-white/5 rounded-lg"
                        >
                          {item.name}
                        </Link>
                      )}
                ...
                <div className="p-4 border-t border-white/20">
                  ...
                  {showSearchResults && !isSearching && (
                    <div className="mt-3">
                      {searchResults.length > 0 ? (
                        <div className="bg-gray-dark border-2 border-white/20 rounded-lg overflow-hidden">
                          {searchResults.map((product) => (
                            <Link
                              key={product.id}
                              href={`/product/${product.slug}`}
